...
def run_sparse_tuning(loader_dict, net, meta_optimizer, epoch):
...
def tune_train_test(tune_train_loader, tune_test_loader, net, meta_optimizer, epoch, sparsity_mode):
    ...
    for c in range(args['tuning_epochs']):
        ...
        if (c + 1) % args['tuning_freq'] == 0:
            ...
            with torch.no_grad():
                ...
                # Computing metrics.
                iou = metrics.jaccard_score(labs_np, prds_np, average="weighted")
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(tune_test_loader):
            ...
            if epoch == args['epoch_num']:
                ...
                io.imsave(os.path.join(outp_path, exp_name, sparsity_mode + '_test_epoch_' + str(epoch),
                                       img_name[0] + '.png'),
                          stored_pred)
